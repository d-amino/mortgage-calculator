    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.button_calculator:
                ...
                int n = freq * year;
                double c = rate / 12;
                txtEnterAmount = findViewById(R.id.enterAmount);
                int amount = Integer.parseInt(txtEnterAmount.getText().toString());
                double totPay = amount * c *Math.pow(1 + c, n)/(Math.pow(1 + c, n) - 1);
                String totalPay = new DecimalFormat("#.0#").format(totPay);
                total = findViewById(R.id.total);
                total.setText("The Payment you need to make is: $" + (totalPay));
                break;
        }
    }
}